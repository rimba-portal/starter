...
if (php_sapi_name() !== 'cli') {
    exit("This script can only be run via the command line interface.\n");
}
...
$prefix = 'update';
if (strpos($branch, 'feat') !== false) {
    $prefix = 'feat';
}
if (strpos($branch, 'fix') !== false || strpos($branch, 'bug') !== false) {
    $prefix = 'fix';
}
if (strpos($branch, 'doc') !== false) {
    $prefix = 'docs';
}
if (strpos($branch, 'refactor') !== false) {
    $prefix = 'refactor';
}
...
$summaryItems = [];
if (! empty($added)) {
    $summaryItems[] = 'add '.implode(', ', $added);
}
if (! empty($modified)) {
    $summaryItems[] = 'modify '.implode(', ', $modified);
}
if (! empty($deleted)) {
    $summaryItems[] = 'remove '.implode(', ', $deleted);
}
